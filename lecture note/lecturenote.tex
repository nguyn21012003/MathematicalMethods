...
\end{gather*}
so that $(f,g)_{w} = (f,wg) = (wf,g)$. The conditions on the weight function are chosen so that the innerproduct remains non-degenerate, i.e. $(f,f)_{w} = 0$ implies $f=0$. We take the weight function to be fixed for each problem. By usual this
